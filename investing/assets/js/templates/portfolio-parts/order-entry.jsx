...
export default function OrderEntry(props) {
  let style = {
    ...
    status: {color: props.order.status == "pending" ? "darkorange" : "inherit"},
  };
  ...
  return (
    <tr>
      ...
      <td style={style.status}>{props.order.status}</td>
      <td>
        {
          props.order.status == "pending" &&
          <button type="button" style={style.close_btn} aria-label="Close" onClick={ () => props.setOrderToCancel(props.order) } data-toggle="modal" data-target="#confirmationModal">
